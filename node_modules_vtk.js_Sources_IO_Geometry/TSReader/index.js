...
//
// Tsurf file reader (Gocad's file format)
//
// based on 
// ...node_modules/vtk.js/Sources/IO/Misc/OBJReader/index.js
// awk '$1~"VRTX" {print "v",$3,$5,-$4} $1~"TRGL" {print "f",$2,$3,$4}' test.ts > test.obj 
// * note: awk's indexing is one-off from javascript's indexing
// * TODO: should have a Tsurf header processing added in the future
//
...
function parseLine(line) {
  ...
  if (tokens[0] === data.splitOn) {
    ...
    data.f.push([]);
    data.size++;
  } else if (tokens[0] === 'VRTX') {
    data.v.push(Number(tokens[2]));
    data.v.push(Number(tokens[4]));
    data.v.push(-1 * Number(tokens[3]));
  } else if (tokens[0] === 'vt') {
...
function vtkTSReader(publicAPI, model) {
  // Set our className
  model.classHierarchy.push('vtkTSReader');
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  // Object methods
  vtkTSReader(publicAPI, model);
}

// ----------------------------------------------------------------------------

export const newInstance = macro.newInstance(extend, 'vtkTSReader');
